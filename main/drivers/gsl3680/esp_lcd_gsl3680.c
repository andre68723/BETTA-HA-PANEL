 ...
 *
 * Source: sukesh-ak/JC8012P4A1-GUITION-ESP32-P4_ESP32-C6 (Guition reference repo,
 *   1-Demo/arduino-examples/.../src/touch/esp_lcd_gsl3680.c), adapted here for
 *   ESP-IDF i2c_master + esp_lcd_touch. The upstream repo declares no license;
 *   the driver originates from Silead reference code for the GSL16xx/GSL3680
 *   controller family. A related GPLv3 port exists in kvj/esphome
 *   (esphome/components/gsl3680).
 */
#include <stdio.h>
#include <string.h>
...
esp_err_t esp_lcd_touch_new_i2c_gsl3680(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    ESP_GOTO_ON_ERROR(ret, err, TAG, "GSL3680 config read failed");
    ...
    ESP_GOTO_ON_ERROR(ret, err, TAG, "GSL3680 firmware verification failed");
...
// #endif
    ...
    ESP_RETURN_ON_ERROR(err, TAG, "GSL3680 touch read failed");
    Finger_num = touch_data[0] & 0x0f;
    if (Finger_num > 2) {
        ESP_LOGW(TAG, "GSL3680 reported %u touches; clamping to the two points read by this driver",
            (unsigned)Finger_num);
        Finger_num = 2;
    }
...
static bool esp_lcd_touch_gsl3680_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
    ...
    uint8_t n = Finger_num < max_point_num ? Finger_num : max_point_num;
    if (n > MAX_FINGER_NUM) {
        n = MAX_FINGER_NUM;
    }
    *point_num = n;
...
static esp_err_t esp_lcd_touch_gsl3680_init(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_touch_gsl3680_clear_reg(tp), TAG, "GSL3680 clear registers failed");
    ESP_RETURN_ON_ERROR(touch_gsl3680_reset(tp), TAG, "GSL3680 reset failed");
    ESP_RETURN_ON_ERROR(esp_lcd_touch_gsl3680_load_fw(tp), TAG, "GSL3680 firmware load failed");
    ESP_RETURN_ON_ERROR(esp_lcd_touch_gsl3680_startup_chip(tp), TAG, "GSL3680 startup failed");
    ESP_RETURN_ON_ERROR(touch_gsl3680_reset(tp), TAG, "GSL3680 post-load reset failed");
    ESP_RETURN_ON_ERROR(esp_lcd_touch_gsl3680_startup_chip(tp), TAG, "GSL3680 post-load startup failed");
...
static esp_err_t touch_gsl3680_reset(esp_lcd_touch_handle_t tp)
{
    ...
    addr = 0xe0;
    write_buf[0] = 0x88;
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp,addr,write_buf,1), TAG, "GSL3680 reset command failed");
    vTaskDelay(pdMS_TO_TICKS(10));
    addr = 0xe4;
    write_buf[0]=0x04;
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp,addr,write_buf,1), TAG, "GSL3680 reset command failed");
    ...
    write_buf[2] =0x00;
    write_buf[3] =0x00;
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp,0xbc,write_buf,4), TAG, "GSL3680 reset command failed");
...
static esp_err_t touch_gsl3680_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp,0xf0,&write,1),TAG,"gsl3680 write error");
    vTaskDelay(pdMS_TO_TICKS(20));
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_read(tp, 0xf0, (uint8_t *)&buf, 4), TAG, "gsl3680 read error!");
...
static esp_err_t esp_lcd_touch_gsl3680_load_fw(esp_lcd_touch_handle_t tp)
{
    ...
    for(source_line=0;source_line<source_len;source_line++)
    {
        ...
        esp_err_t err = (addr == 0xf0)
            ? touch_gsl3680_i2c_write(tp,addr,wrbuf,1)
            : touch_gsl3680_i2c_write(tp,addr,wrbuf,4);
        ESP_RETURN_ON_ERROR(err, TAG, "GSL3680 firmware write failed");
        if ((source_line % 256U) == 0U) {
...
static esp_err_t esp_lcd_touch_gsl3680_clear_reg(esp_lcd_touch_handle_t tp)
{
    ...
    addr = 0xe0;
    wrbuf[0] = 0x88;
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp,addr,wrbuf,1), TAG, "GSL3680 clear register failed");
    vTaskDelay(pdMS_TO_TICKS(20));
    addr = 0x88;
    wrbuf[0] = 0x01;
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_write(tp,addr,wrbuf,1), TAG, "GSL3680 clear register failed");
